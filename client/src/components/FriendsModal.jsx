...
const preferenceLabels = [
  ...
  ['showGame', 'Afficher « Dans le lobby » ou « En partie » dans mon statut'],
        ...
        <div className="sj-friends-layout">
          ...
          <div className="sj-friends-content">
            ...
            <section><h3><span>Mes amis</span><span className="sj-friends-online-count"><i aria-hidden="true" />{onlineCount} en ligne</span></h3>{!loaded ? <div className="sj-friends-loading" aria-hidden="true"><span /><span /></div> : friends.length ? <ul className="sj-lobby-player-list sj-friends-list">{friends.map((friend) => {
              ...
              return <li className={`sj-pop-in ${friend.online === false ? 'is-disconnected' : ''}`} key={friend.relationId}><span className="sj-lobby-player-copy"><strong>{friend.name}</strong>{friend.online !== null && <i className="sj-lobby-player-presence" role="img" aria-label={friend.status} title={friend.status} />}</span><span className="sj-lobby-player-badges"><span className={friend.status === 'available' ? 'is-online' : ''}>{friend.status === 'lobby' ? 'Dans le lobby' : friend.status === 'playing' ? 'En partie' : friend.status === 'dnd' ? 'Ne pas déranger' : friend.status === 'hidden' ? 'Masqué' : friend.online ? 'Disponible' : 'Hors ligne'}</span>{invitationPending && <span className="is-invited">Invitation envoyée</span>}</span><span className="sj-friends-player-actions">{friend.online === true && friend.game?.visibility === 'public' && friend.game?.canJoin && actionButton('sj-friends-join', 'Rejoindre', <LogIn aria-hidden="true" size={17} />, () => onJoin(friend.game.roomId))}{!inRoom && friend.online === true && friend.game?.visibility === 'public' && friend.game?.canWatch && actionButton('sj-friends-watch', 'Regarder', <Eye aria-hidden="true" size={17} />, () => onWatch(friend.game.roomId))}{roomId && friend.status !== 'dnd' && !invitationPending && actionButton('sj-friends-invite', 'Inviter', <Gamepad2 aria-hidden="true" size={17} />, () => mutate('invite', { relationId: friend.relationId, roomId }))}{friend.profile && actionButton('sj-friends-profile-action', 'Voir le profil', <BarChart3 aria-hidden="true" size={17} />, () => setProfileFriend(friend))}{actionButton('', 'Retirer', <UserMinus aria-hidden="true" size={17} />, () => setFriendToRemove(friend))}</span></li>;
