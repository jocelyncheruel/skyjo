...
export default function FriendsModal({ open, onClose, onWatch, onJoin, onError, roomId = '', inRoom = false, refreshToken = 0 }) {
  ...
  function toggleFriendMenu(event, relationId, hasProfile) {
    ...
    const menuItemCount = 1 + Number(hasProfile) + Number(standaloneApp);
    const estimatedMenuHeight = compactViewport
      ? 48 + Math.max(0, menuItemCount - 1) * 42
      : 46 + Math.max(0, menuItemCount - 1) * 40;
        ...
        <div className="sj-friends-layout">
          ...
          <div className="sj-friends-content" onScroll={() => setFriendMenu(null)}>
            ...
            <section><h3><span>Mes amis</span><span className="sj-friends-online-count"><i aria-hidden="true" />{onlineCount} en ligne</span></h3>{!loaded ? <div className="sj-friends-loading" aria-hidden="true"><span /><span /></div> : friends.length ? <ul className="sj-lobby-player-list sj-friends-list">{friends.map((friend) => {
              ...
              const friendIsInCurrentRoom = friend.inSameRoom === true
                || Boolean(roomId && friend.game?.roomId === roomId);
              const menuOpen = friendMenu?.relationId === friend.relationId;
              return <li className={`sj-pop-in ${friend.online === false ? 'is-disconnected' : ''} ${menuOpen ? 'is-menu-open' : ''}`} key={friend.relationId}><span className="sj-lobby-player-copy"><strong>{friend.name}</strong>{friend.online !== null && <i className="sj-lobby-player-presence" role="img" aria-label={friend.status} title={friend.status} />}</span><span className="sj-lobby-player-badges"><span className={friend.status === 'available' ? 'is-online' : ''}>{friend.status === 'lobby' ? 'Dans le lobby' : friend.status === 'playing' ? 'En partie' : friend.status === 'dnd' ? 'Ne pas déranger' : friend.status === 'hidden' ? 'Masqué' : friend.online ? 'Disponible' : 'Hors ligne'}</span>{invitationPending && !friendIsInCurrentRoom && <span className="is-invited">Invitation envoyée</span>}</span><span className="sj-friends-player-actions">{!friendIsInCurrentRoom && friend.online === true && friend.game?.visibility === 'public' && friend.game?.canJoin && actionButton('sj-friends-join', 'Rejoindre', <LogIn aria-hidden="true" size={17} />, () => onJoin(friend.game.roomId))}{!friendIsInCurrentRoom && !inRoom && friend.online === true && friend.game?.visibility === 'public' && friend.game?.canWatch && actionButton('sj-friends-watch', 'Regarder', <Eye aria-hidden="true" size={17} />, () => onWatch(friend.game.roomId))}{roomId && !friendIsInCurrentRoom && friend.status !== 'dnd' && !invitationPending && actionButton('sj-friends-invite', 'Inviter', <Gamepad2 aria-hidden="true" size={17} />, () => mutate('invite', { relationId: friend.relationId, roomId }))}<span className="sj-friends-more"><button type="button" className="sj-lobby-player-remove sj-friends-more-trigger" aria-label={`Plus d’actions pour ${friend.name}`} aria-haspopup="menu" aria-expanded={menuOpen} aria-controls={`friend-actions-${friend.relationId}`} onClick={(event) => toggleFriendMenu(event, friend.relationId, Boolean(friend.profile))}><Ellipsis aria-hidden="true" size={18} /></button>{menuOpen && createPortal(<span id={`friend-actions-${friend.relationId}`} className={`sj-friends-more-menu ${friendMenu.opensUp ? 'opens-up' : ''}`} style={{ right: friendMenu.right, top: friendMenu.top }} role="menu">{standaloneApp && <button type="button" className={friend.notifyOnline ? 'is-active' : ''} role="menuitemcheckbox" aria-checked={friend.notifyOnline} onClick={() => { setFriendMenu(null); toggleOnlineNotification(friend.relationId, friend.notifyOnline); }}><Bell aria-hidden="true" size={16} /><span>{friend.notifyOnline ? 'Ne plus signaler sa connexion' : 'Me prévenir lorsqu’il est en ligne'}</span></button>}{friend.profile && <button type="button" role="menuitem" onClick={() => { setFriendMenu(null); setProfileFriend(friend); }}><BarChart3 aria-hidden="true" size={16} /><span>Voir les statistiques et la progression</span></button>}<button type="button" className="is-danger" role="menuitem" onClick={() => { setFriendMenu(null); setFriendToRemove(friend); }}><UserMinus aria-hidden="true" size={16} /><span>Retirer l’ami</span></button></span>, document.body)}</span></span></li>;
